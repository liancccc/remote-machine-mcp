...
## Testing Guidelines
Write table-driven Go tests in `*_test.go` files next to the code they cover. Name tests with standard Go patterns such as `TestHTTPServerRejectsBadAuth` or `TestReadFileRejectsEscapes`. Cover success paths and boundary checks, especially path-guarding, file mutation behavior, and long-running command/session handling. Run `go test ./...` before opening a PR.
...
Do not commit bearer tokens, host-specific paths, or temporary working data. This project currently does not enforce allowed-root restrictions; assume tools can access any path visible to the server process. HTTP mode requires bearer auth; pass `--token` explicitly or use the generated token printed at startup.
